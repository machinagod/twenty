...
Remaining boot blocker — **GraphQL duplicate `@ObjectType` ("Schema must contain uniquely named types but contains multiple types named X")**. Precisely diagnosed (instrumentation in node_modules, since reverted):
- The DTO file loads **once**; there is only **one** `@ObjectType('RowLevelPermissionPredicateGroup')` in source; `@nestjs/graphql` is symlink-deduped to one variant. Not a source double-eval.
- `type-definitions.generator.generate()` runs **twice** (Twenty builds 2+ schemas; both `scope=undefined` — the patch's `resolverSchemaScope` isn't taking effect on Deno). Within a build, `object-type-definition.factory.create()` is called **multiple times for the SAME class** (`sameObj=true`) — once via normal traversal, once via the **orphaned-types** list (`orphaned=1`). So `@nestjs/graphql`'s per-build type cache (keyed by target class) is **not deduping on Deno** → two `GraphQLObjectType`s with the same name → graphql throws at `GraphQLSchema` assembly.
- Likely cause = **a second `@nestjs/graphql` instance** so the reachable path and orphaned path use different `TypeDefinitionsStorage`/`TypeMetadataStorage` singletons. Partial fix applied: `gen-import-map.py` now maps deep npm subpaths to the **node_modules file path** (same instance) instead of `npm:` (which spins a separate instance). It did NOT fully resolve the dup, so **there is still another instance-split source to find** (candidate: `@graphql-yoga/nestjs`, or a shim, importing `@nestjs/graphql` via a different specifier). NEXT: instrument `TypeDefinitionsStorage`/`TypeMetadataStorage` construction to count instances at runtime; or `console.error(import.meta.url)` at the top of a `@nestjs/graphql` dist file to see if it loads under two URLs.
- After that: **DB un-migrated** (`relation "core.appToken" does not exist` — non-fatal; config falls back to env; needs `database:init` on Deno), then HTTP via **`Deno.serve`** + the **`Deno.cron`** drain/schedule heartbeats.
...
3. `bash deno-spike/apply-patches.sh` (applies the 3 Yarn patches to node_modules).
4. Postgres with a `core` + `metadata` schema; set `PG_DATABASE_URL`.
5. From `packages/twenty-server/`: `env NODE_ENV=development APP_SECRET=… PG_DATABASE_URL=… MESSAGE_QUEUE_DRIVER_TYPE=pg CACHE_STORAGE_TYPE=memory SESSION_STORAGE_TYPE=memory PUB_SUB_DRIVER_TYPE=postgres deno run -A --sloppy-imports <entry importing AppModule>`.
